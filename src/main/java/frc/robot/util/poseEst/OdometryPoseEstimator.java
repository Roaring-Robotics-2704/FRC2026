package frc.robot.util.poseEst;
...
/**
 * A mostly copy-paste implementation of the WPILib pose estimator that
 * publicizes the internal odometry pose buffer.
 * 
 * This class wraps {@link Odometry} to fuse latency-compensated vision
 * measurements with encoder measurements. Robot
 * code should not use this directly- Instead, use the particular type for your
 * drivetrain (e.g.,
 * {@link DifferentialDrivePoseEstimator}). It is intended to be a drop-in
 * replacement for {@link Odometry}; in fact, if
 * you never call {@link OdometryPoseEstimator#addVisionMeasurement} and only
 * call {@link OdometryPoseEstimator#update}
 * then this will behave exactly the same as Odometry.
 *
 * <p>
 * {@link OdometryPoseEstimator#update} should be called every robot loop.
 *
 * <p>
 * {@link OdometryPoseEstimator#addVisionMeasurement} can be called as
 * infrequently as you want; if you never call it
 * then this class will behave exactly like regular encoder odometry.
 *
 * @param <T>
 *            Wheel positions type.
 */
public class OdometryPoseEstimator<T> {
    public final Odometry<T> m_odometry;
    private final Matrix<N3, N1> m_q = new Matrix<>(Nat.N3(), Nat.N1());
    private final Matrix<N3, N3> m_visionK = new Matrix<>(Nat.N3(), Nat.N3());

    private static final double kBufferDuration = 1.5;
    // Maps timestamps to odometry-only pose estimates
    public final TimeInterpolatableBuffer<Pose2d> m_odometryPoseBuffer = TimeInterpolatableBuffer
     ...
     *            position in meters, and
     *            heading in radians). Increase these numbers to trust your state
     *            estimate less.
     * @param visionMeasurementStdDevs
     *            Standard deviations of the vision pose measurement (x position in
     *            meters, y
     *            position in meters, and heading in radians). Increase these
     *            numbers to trust the vision pose
     *            measurement less.
     */
    @SuppressWarnings("PMD.UnusedFormalParameter")
    public OdometryPoseEstimator(Kinematics<?, T> kinematics, Odometry<T> odometry, Matrix<N3, N1> stateStdDevs,
     ...
     * change trust in vision measurements
     * after the autonomous period, or to change trust as distance to a vision
     * target increases.
     *
     * @param visionMeasurementStdDevs
     ...
     * The library automatically takes care
     * of offsetting the gyro angle.
     *
     * @param gyroAngle
     ...
     * odometry pose estimate while still
     * accounting for measurement noise.
     *
     * <p>
     * This method can be called as infrequently as you want, as long as you are
     * calling
     * {@link OdometryPoseEstimator#update} every loop.
     *
     * <p>
     * To promote stability of the pose estimate and make it robust to bad vision
     * data, we recommend only adding vision
     * measurements that are already within one meter or so of the current pose
     * estimate.
     *
     * @param visionRobotPoseMeters
     *            The pose of the robot as measured by the vision camera.
     * @param timestampSeconds
     *            The timestamp of the vision measurement in seconds. Note that if
     *            you don't use your own
     *            time source by calling
     *            {@link OdometryPoseEstimator#updateWithTime(double,Rotation2d,Object)}
     *            then you
     *            must use a timestamp with an epoch since FPGA startup (i.e., the
     *            epoch of this timestamp is the same
     *            epoch as {@link edu.wpi.first.wpilibj.Timer#getFPGATimestamp()}.)
     *            This means that you should use
     *            {@link edu.wpi.first.wpilibj.Timer#getFPGATimestamp()} as your
    ...
    public void addVisionMeasurement(Pose2d visionRobotPoseMeters, double timestampSeconds) {
        ...
        // timespan, skip.
        if (m_odometryPoseBuffer.getInternalBuffer().isEmpty()
                || m_odometryPoseBuffer.getInternalBuffer().lastKey() - kBufferDuration > timestampSeconds) {
            return;
        }
     ...
     * will continue to apply to future
     * measurements until a subsequent call to
     * {@link OdometryPoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this
     * method.
     *
     * @param visionRobotPoseMeters
     *            The pose of the robot as measured by the vision camera.
     ...
     *            The timestamp of the vision measurement in seconds. Note that if
     *            you don't use your own
     *            time source by calling {@link #updateWithTime}, then you must use
     *            a timestamp with an epoch since FPGA
     *            startup (i.e., the epoch of this timestamp is the same epoch as
     *            {@link edu.wpi.first.wpilibj.Timer#getFPGATimestamp()}). This
     ...
     * pose estimate as well as the
     * corresponding odometry pose estimate.
     */
    private static final class VisionUpdate {
         ...
         *            The vision-compensated pose estimate.
         * @param odometryPose
         *            The pose estimate based solely on odometry.
         */
         ...
         * pose from being relative to
         * this record's odometry pose to being relative to this record's vision pose.
         *
         * @param pose
         *            The pose to compensate.
