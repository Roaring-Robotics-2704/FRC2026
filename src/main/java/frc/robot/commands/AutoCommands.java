    ...
    public static Command returnToClosestLaunchPose(Drive drive) {
        return new DriveToPose(
                ...
                () -> {
                    ...
                    Translation2d fieldVelocityTranslation = new Translation2d(fieldVelocity.vxMetersPerSecond,
                            fieldVelocity.vyMetersPerSecond);
                    Translation2d currentTranslation = RobotState.getInstance()
                            .getEstimatedPose()
                            .getTranslation()
                            .plus(fieldVelocityTranslation.times(0.8));
                    Pose2d target = currentTranslation.getDistance(
                            AllianceFlipUtil.apply(
                                    AutoFieldConstants.Launch.leftBump.getTranslation())) < currentTranslation
                                            .getDistance(
                                                    AllianceFlipUtil.apply(
                                                            AutoFieldConstants.Launch.rightBump.getTranslation()))
                                                                    ? AllianceFlipUtil
                                                                            .apply(AutoFieldConstants.Launch.leftBump)
                                                                    : AllianceFlipUtil
                                                                            .apply(AutoFieldConstants.Launch.rightBump);
    ...
    public static Command returnToDeterminedLaunchPose(Drive drive, AutoQuestionResponse bumpSide) {
        return new DriveToPose(
                ...
                () -> {
                    ...
                    Pose2d target = bumpSide.equals(AutoQuestionResponse.LEFT_BUMP)
